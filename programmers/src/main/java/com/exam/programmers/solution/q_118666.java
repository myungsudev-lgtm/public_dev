...
////        survey: 문항; "RT", "TR", "FC", "CF", "MJ", "JM", "AN", "NA"
////        choices: 응답; choices의 길이 = survey의 길이
...
//                answer.add(type);
//            }else {
//                String type = String.valueOf(survey[i].charAt(1));
//                answer.add(type);
//            }
//        }
////    1번 지표	라이언형(R), 튜브형(T)
...
//
//        for (int i = 0; i < Math.min(survey.length, choices.length); i++) {
...
//            if (choice < 4) { // 비동의 → 왼쪽
//                int times = 4 - choice;
//                for (int t = 0; t < times; t++) picked.add(left);
//            } else if (choice > 4) { // 동의 → 오른쪽
//                int times = choice - 4;
//                for (int t = 0; t < times; t++) picked.add(right);
//            }
//        }
//
//        // 문자별 점수 계산
...
//        }
//
//        // StringBuilder 대신 String 직접 연결
//        String result = "";
//        result += (score.get('R') >= score.get('T')) ? "R" : "T";
//        result += (score.get('C') >= score.get('F')) ? "C" : "F";
//        result += (score.get('J') >= score.get('M')) ? "J" : "M";
//        result += (score.get('A') >= score.get('N')) ? "A" : "N";
//
//        return result;
//    }
//}